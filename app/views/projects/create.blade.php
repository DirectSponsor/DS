{{ Form::open(array('route'=>'projects.store')) }}
    ...
    <div class="clear"></div>
    {{ Form::label('password',' Coordinator\'s password :'), Form::text('password',Input::old('password')) }}
    @if($errors->has('password')) {{ $errors->first('password','<p class="remarques"> :message </p>') }} @endif
    <div class="clear"></div>
    {{ Form::label('content','Content :')}} 
        <div class="clear"></div>
    {{ Form::textarea('content',Input::old('content'),array('class'=>'ckeditor full')) }} 
        @if($errors->has('content')) {{ $errors->first('content','<p class="remarques"> :message </p>') }} @endif
        <div class="clear"></div>
